...
class ModelSchema(with_metaclass(ModelSchemaMeta, ma.Schema)):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.session = session or self.opts.sqla_session  # pylint: disable=no-member

    def get_instance(self, data):
        """Retrieve an existing record by primary key(s)."""
        props = get_primary_keys(self.opts.model)  # pylint: disable=no-member
        filters = {
            prop.key: data.get(prop.key)
            for prop in props
        }
        if None not in filters.values() and self.session is not None:
            return self.session.query(
                self.opts.model  # pylint: disable=no-member
            ).filter_by(
                **filters
            ).first()
        return None
    ...
    def make_instance(self, data):
        ...
        if instance is not None:
            ...
            return instance
        return self.opts.model(**data)  # pylint: disable=no-member
